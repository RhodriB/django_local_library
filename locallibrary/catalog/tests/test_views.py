...
class AuthorCreateViewTest(TestCase):
    def setUp(self):
        ...
        # Give test_user1 permission to add_author.
        ...
        testAuthor = Author.objects.create(first_name='John', last_name='Smith')
    ...
    def test_uses_correct_template(self):
        ...
        response = self.client.get(reverse('author-create'))
        self.assertTrue(response.status_code, 200)
        self.assertTemplateUsed('catalog/templates/catalog/author_detail.html')
    
    def test_form_renewal_date_initially_has_date_of_death(self):
        login = self.client.login(username='testuser2', password='2HJ1vRV0Z&3iD')
        response = self.client.get(reverse('author-create'))
        self.assertEqual(response.status_code, 200)
    ...
    def test_redirects_to_author_detail_on_success(self):
        login = self.client.login(username='testuser2', password='2HJ1vRV0Z&3iD')
        response = self.client.post(reverse('author-create'), {'first_name':'Bob', 'last_name':'Jones' },)
        self.assertEqual(response.status_code, 302)
